...
이렇게 개선하면 DB I/O 횟수를 줄이고 병목현상을 해결할 수 있습니다. 또한 저장되는 로그를 활용해서 추가되는 요구사항에 대응하기 쉬워집니다.
(e.g 날짜별 인기 검색어, 검색 로그 기록)
...
> 현재 구현상태는 로그를 메모리(자바 자료구조)에 저장하고 있어서 Lock 때문에 병목이 발생할 가능성은 작습니다. 하지만 File System이나 캐시에 저장하는 걸 목표로
> 구현하고 있었기
...
1) interface에 맞게 Google API 요청 클래스를 구현한다.
2) 비즈니스 로직에서 Google API 세부 사항을 알필요 없이 호출한다. 응답은 다른 API와 같은 포맷으로 받을 수 있다.
...
이슈: 트래픽이 증가하면 외부 API 호출과 DB 호출 횟수도 증가합니다. 그리고 외부 서비스 I/O 처리 시간에 많은 영향을 받게 됩니다.

개선:  
검색, 인기 검색어 조회 API 모두 캐시를 적용했습니다.

#### a) 검색 API

검색 조회 결과는 캐시에 저장됩니다. 같은 쿼리로 재요청하면 캐시에서 데이터를 가져오기 때문에 외부 API 호출과 정렬을 하지 않아도 됩니다.
만약 유료 버전을 사용하고 있다면 비용을 아낄 수 있는 전략이기도 합니다.

#### b) 인기 검색어

인기 검색어를 구하는 쿼리는 데이터양이 많을 수록 처리 비용이 커질 수 있습니다.

![popular-query-sys-design](images/popular-query-sys-design.png)

워커는 주기적으로 DB 데이터를 캐싱합니다. 그리고 API는 캐시를 통해 가져옵니다.