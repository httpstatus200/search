...
- [테스트 방법](#테스트-방법)
- [시스템 디자인](#시스템-디자인)
- [사용한 외부 라이브러리 및 오픈소스](#사용한-외부-라이브러리-및-오픈소스)
    - [h2 database](#h2-database)
    - [ehcache3](#ehcache3)
    - [Lombok](#lombok)
- [기술적 요구사항 구현](#기술적-요구사항-구현)
    - [1) 키워드 검색 횟수](#1-키워드-검색-횟수)
    - [2) 새로운 검색 API 추가 시 변경 영역 최소화에 대한 고려](#2-새로운-검색-api-추가-시-변경-영역-최소화에-대한-고려)
    - [3) 캐시를 활용한 API 성능 개선](#3-캐시를-활용한-api-성능-개선)
...
이슈: API 요청마다 DB에서 `UPDATE table_name SET count = count + 1` 쿼리를 사용하여 카운트를 업데이트할 수 있습니다. 하지만
...
1) 발생한 키워드 로그를 메시지 큐에 전달합니다.
2) Query Log System 은 큐에서 로그를 받아 Log Store에 저장합니다.
3) 주기적으로 실행되는 워커는 Log Store에서 로그를 읽은 다음 키워드별로 카운팅 후 DB에 저장합니다.

이렇게 카운팅을 비동기로 처리하면 앞서 언급한 DB I/O 횟수를 줄이고 병목현상을 개선할 수 있습니다. 또한 키워드 로그를 활용한 요구사항이 추가될 때,
로그 시스템을 통해 로그 처리 기능을 확장할 수 있습니다. (e.g 날짜별 인기 검색어, 검색 로그 기록)

근데 여기서 `왜 로그를 메시지 큐와 쿼리 로그 시스템을 통해 저장하지?` 라는 의문이 생길 수 있습니다. 이유는 이렇습니다.

#### a) 현재 저장된 로그를 워커가 읽고 제거하기 때문에 동기화가 필요합니다.

현재 로그는 메모리(List 자료구조)에 저장되고 있습니다. 그리고 워커는 이 로그를 읽고 카운팅하는 과정에서 읽은 로그를 제거합니다.
이때 데이터 유실이 발생할 수 있기 때문에 로그 저장과 삭제 사이에 동기화가 필요합니다.

동기화를 위해 Lock을 사용합니다. 이때 API에서 직접 Store에 데이터를 추가한다면 Lock으로 인해 병목이 발생할 수 있습니다. 이런 동기화 처리를 다른 서비스에서
비동기로 처리하여 병목구간을 개선했습니다.

> 현재 구현상태는 로그를 메모리(List 자료구조)에 저장하고 있어서 Lock 때문에 병목이 발생할 가능성은 적습니다. 하지만 File System이나 in-memory
> DB 사용을 고려할 필요가 있기 때문에 먼저 이와 같은 구조를 구성했습니다.
>
> 실제 ehcache3를 사용하도록 로직을 변경해서 JMeter로 테스트했을때 아래 결과를 확인할 수 있었습니다.
>
> <직접 저장>
...
#### b) 로그 시스템의 역할이 커졌을 때 현재 구조의 장점이 있습니다.

만약 로그 시스템의 역할이 커지면 서비스로 분리할 수 있습니다. 이때 큐를 통해 로그를 전달하게 되면 로그 서비스의 지연이나 장애 전파를 예방할 수 있습니다.
